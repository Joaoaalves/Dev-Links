...
async function POST(req, res, session){
  try {
    ...
    await User.findOneAndUpdate({ email: session.user.email }, {firstName, lastName, image}, {new: true}); 
...
async function GET(res, session){
  try {
    await connectDB()
    const user = await User.findOne({ email: session.user.email }, { password: 0 }).populate('links');
