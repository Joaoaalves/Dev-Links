...
async function createOrUpdateLinks(links) {
    return await Promise.all(
      links.map(async (link) => {
        // Check if link already exists based on valid MongoDB _id length
        ...
        // Extract data, excluding _id to let MongoDB generate a new one
        const { _id, ...linkData } = link; 
  
        // Create the new link and return its _id
